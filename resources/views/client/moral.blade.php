...
@section('content')
        ...
        <div class="flex flex-col bg-[#F2EDF3] w-full h-full m-4">
            ...
            <div class="relative mt-4 ml-6 m-4">
                <form action="{{ route('client.moral') }}" method="GET" class="flex relative">
        ...
        <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                        <form action="{{ route('client.moral.store') }}" method="POST" class="p-4 md:p-5" id="formulaire">
                            @csrf
                                ...
                                <div class="flex flex-row ml-64">
                                    <button type="button" class="bg-[#53ABDC] text-white py-4 mr-2 rounded-lg" onclick="resetForm()">Annuler</button>
                    ...
                    <tr class="space-y-4 border-b border-[#7D7272] border-opacity-40">
                        ...
                        <td>
                            <div onclick="event.stopPropagation();" class="flex items-center justify-center gap-1">
                                <a href="#" data-modal-target="crud-modale-{{ $personneMorale->id }}" data-modal-toggle="crud-modale-{{ $personneMorale->id }}" >
                                    ...
                                    <div onclick="event.stopPropagation();" id="crud-modale-{{ $personneMorale->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <!-- Modal content -->
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" onclick="event.stopPropagation();">
                                                ...
                                                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                                    ...
                                                    <button type="button" class="text-gray-400 bg-[#EF2549] hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="crud-modale-{{ $personneMorale->id }}">
                                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                        </svg>
                                                        <span class="sr-only">Close modal</span>
                                                ...
                                                <!-- Modal body -->
                                                <form action="{{ route('client.moral.update', ['id' => $personneMorale->id]) }}" method="POST" class="p-4 md:p-5" id="formulaire">
                                                    @csrf
                                                        @method('PUT')
                                                            ...
                                                            <div class="flex flex-row ml-64">
                                                                <button type="button" class="bg-[#53ABDC] text-white py-4 mr-2 rounded-lg" data-modal-toggle="crud-modale-{{ $personneMorale->id }}">Annuler</button>
                                                                <button type="submit" class="bg-[#DCB253] text-white py-4 rounded-lg">Enrégistrer</button>
                                                            </div>
                                            </form>
                                            </div>
                                ...
                                <a href="#"  data-modal-target="popup-modal-{{ $personneMorale->id }}" data-modal-toggle="popup-modal-{{ $personneMorale->id }}">
                                    ...
                                    <div id="popup-modal-{{ $personneMorale->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                                    ...
                                                    <button type="button" class="text-gray-400 bg-[#EF2549] hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" onclick="closeModal('popup-modal-{{ $personneMorale->id }}')">
                                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                        </svg>
                                                        <span class="sr-only">Close modal</span>
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <form action="{{ route('client.moral.destroy', ['id' => $personneMorale->id]) }}" method="POST" onclick="event.stopPropagation();">
                                                        @csrf
                                                        @method('DELETE')

                                                        <button type="button" onclick="closeModal('popup-modal-{{ $personneMorale->id }}')" class="text-white bg-[#53ABDC] hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2">
                                                            Annuler
                                                        </button>
...
<div class="flex items-center justify-between mt-4 ml-6 m-4">
    <div class="flex items-center bg-white rounded-lg ml-auto mr-80">
        <div class="relative px-1 py-1 mr-2">
            <!-- Bouton "Précédent" -->
            @if ($personnesMorales->previousPageUrl())
                <a href="{{ $personnesMorales->previousPageUrl() }}" class="h-11 flex items-center px-4">Previous</a>
            @else
        ...
        <div class="flex space-x-2">
            @for ($i = 1; $i <= $personnesMorales->lastPage(); $i++)
                <div class="border-l h-14 mx-2 border-[#7D7272] opacity-20"></div>
                <div class="relative px-1 py-1">
                    <!-- Bouton de numéro de page -->
                    <button class="h-11 rounded-md flex items-center {{ $personnesMorales->currentPage() == $i ? 'bg-gray-500 text-white' : '' }}">
                        {{ $i }}
                    </button>
                </div>
            @endfor
        ...
        <div class="relative px-2 py-1 ml-2">
            <!-- Bouton "Suivant" -->
            @if ($personnesMorales->nextPageUrl())
                <a href="{{ $personnesMorales->nextPageUrl() }}" class="h-11 rounded-md flex items-center">Next</a>
            @else
